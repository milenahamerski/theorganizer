...
def list_books():
    ...
    cur.close()
    conn.close()
    return [Book(id=r[0], title=r[1], author=r[2], year=r[3], image_url=r[4]) for r in rows]
...
def get_book(book_id: int):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, title, author, year, image_url FROM books WHERE id = %s", (book_id,))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if not row:
...
def update_book(book_id: int, book: Book):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE books SET title = %s, author = %s, year = %s, image_url = %s WHERE id = %s",
